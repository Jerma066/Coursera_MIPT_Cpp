...
namespace Json {
  ...
  void PrintNodeString(const Node& node, std::ostream& output){
      output << "\"" << node.AsString() << "\"";
  }

  void PrintNodeInt(const Node& node, std::ostream& output){
      output << node.AsInt();
  }

  void PrintNodeDouble(const Node& node, std::ostream& output){
  ...
  void PrintNode(const Node& node, std::ostream& output){
      if(node.HasVector()){
          ...
          throw std::invalid_argument("Node does not contain any value");
      }
  }
}
