...
public:
	...
	void Add(const Type& value){
		size_t i = GetHashIndex(value);
		if(!Has(value)){
			table[i].push_front(value);
		}
	}
		
	bool Has(const Type& value) const{
		size_t i = GetHashIndex(value);
		return find(table[i].begin(), table[i].end(), 
					value) != table[i].end();
	}
...
private:
	size_t GetHashIndex(const Type& value) const {
		return hash_func(value) % table.size();
	}
